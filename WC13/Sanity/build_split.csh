...
#                                                                       :::
#                  build_split.csh -ad -j 10 -b feature/kernel          :::
#                                                                       :::
...
if ( $ad_exe == 1 ) then
  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DADM_DRIVER"
  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DINITIALIZE_AUTOMATIC"

  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DFORWARD_FLUXES"
  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DFORWARD_READ"

# setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DAD_OUTPUT_STATE"
...
if ( $nl_exe == 1 ) then
  ...
  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DSOLAR_SOURCE"

  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DLIMIT_VDIFF"
  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DLIMIT_VVISC"
...
if ( $tl_exe == 1 ) then
  setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DTLM_DRIVER"
...
if ( $pio_lib == 1 ) then
 ...
 setenv MY_CPP_FLAGS "${MY_CPP_FLAGS} -DFORWARD_MIXING"
...
if ( $?USE_DEBUG ) then
  if ( $ad_exe == 1 ) then
    setenv BIN              ${BINDIR}/romsG_ad
  else if ( $tl_exe == 1 ) then
    setenv BIN              ${BINDIR}/romsG_tl
  else
  ...
  if ( $?USE_MPI ) then
    if ( $ad_exe == 1 ) then
      setenv BIN            ${BINDIR}/romsM_ad
    else if ( $tl_exe == 1 ) then
      setenv BIN            ${BINDIR}/romsM_tl
    else
      setenv BIN            ${BINDIR}/romsM_nl
    endif
  else
    if ( $ad_exe == 1 ) then
      setenv BIN            ${BINDIR}/romsS_ad
    else if ( $tl_exe == 1 ) then
      setenv BIN            ${BINDIR}/romsS_tl
    else
      setenv BIN            ${BINDIR}/romsS_nl
    endif
  endif
endif
...
if ($?USE_DEBUG) then
  if ( $ad_exe == 1 ) then
    setenv BUILD_DIR        ${MY_PROJECT_DIR}/Build_romsG_ad
  else if ( $tl_exe == 1 ) then
    setenv BUILD_DIR        ${MY_PROJECT_DIR}/Build_romsG_tl
  else
    setenv BUILD_DIR        ${MY_PROJECT_DIR}/Build_romsG_nl
  endif
else
  if ( $ad_exe == 1 ) then
    setenv BUILD_DIR        ${MY_PROJECT_DIR}/Build_roms_ad
  else if ( $tl_exe == 1 ) then                                                                                     |    elif [ $tl_exe -eq 1 ]; then
    setenv BUILD_DIR        ${MY_PROJECT_DIR}/Build_roms_tl
  else
