...
export   ROMS_APPLICATION=LAKE_DECIMATE
...
if [ $branch -eq 1 ]; then
  ...
  if [[ ${COMPILERS} == ${MY_ROMS_SRC}* ]]; then
    export COMPILERS=${BUILD_DIR}/roms/Compilers
  fi
  export MY_ROMS_SRC=${BUILD_DIR}/roms
